  ...
  <form class="form-horizontal" action="" method="post">
   <fieldset>
    ...
    if (!isset($_POST["OrderExport"])) {
        ...
        $EcsOrderSettings->displayOrderExpSettings(
            $Cron,
            $Path,
            $Shipping,
            $Status,
    ...
    ?>
    <?php if (isset($_POST["OrderExport"])) {
        // handle post data
        $localFile = "test.xml";
        ...
        if ($ftpCheck[0] == "SUCCESS") {
            ...
            if ($settingID == "") {
                ...
                foreach ($statesmeta as $k) {
                    if ($k->keytext == "Cron") {
                        $EcsOrderSettings->updateSettingsValues($k->id, $Cron);
                    }
                    if ($k->keytext == "Path") {
                        $EcsOrderSettings->updateSettingsValues($k->id, $Path);
                    }
                    if ($k->keytext == "Shipping") {
                        $EcsOrderSettings->updateSettingsValues(
                            $k->id,
                            $Shipping
                        );
                    }
                    ...
                    if ($k->keytext == "no") {
                        $EcsOrderSettings->updateSettingsValues($k->id, $no);
                    }
