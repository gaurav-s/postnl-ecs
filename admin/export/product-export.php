  ...
  <form class="form-horizontal" action="" method="post">
   <fieldset>
    ...
    if (!isset($_POST["ProductExport"])) {
        ...
        $EcsProductSettings->displayProductExpSettings(
            $Cron,
            $Path,
            $no,
            $ean_field
    ...
    ?>
    <?php if (isset($_POST["ProductExport"])) {
        // handle post data
        $localFile = "test.xml";
        ...
        if ($ftpCheck[0] == "SUCCESS") {
            ...
            if ($settingID == "") {
                ...
                foreach ($statesmeta as $k) {
                    if ($k->keytext == "Cron") {
                        $EcsProductSettings->updateSettingsValues(
                            $k->id,
                            $Cron
                        );
                    }
                    ...
                    if ($k->keytext == "no") {
                        $EcsProductSettings->updateSettingsValues($k->id, $no);
                    }
                    if ($k->keytext == "ean_attribute") {
                        $EcsProductSettings->updateSettingsValues(
                            $k->id,
                ...
                if (!$eanfieldSet) {
                    $EcsProductSettings->saveSettingsValues(
                        $settingID,
                        "ean_attribute",
                        $ean_field
                    );
