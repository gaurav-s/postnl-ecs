  ...
  <form class="form-horizontal" action="" method="post">
   <fieldset>
    ...
    if (!isset($_POST["shipmentImport"])) {
        ...
        $EcsShipmentSettings->displayShipmentSettings(
            $Cron,
            $Path,
            $tracking,
            $Inform,
    ...
    ?>
    <?php if (isset($_POST["shipmentImport"])) {
        // handle post data
        ...
        if ($ftpCheck[0] == "SUCCESS") {
            ...
            if ($settingID == "") {
                ...
                $EcsShipmentSettings->saveSettingsValues(
                    $id,
                    "tracking",
                    $tracking
                );
                ...
                $statesmeta = $EcsShipmentSettings->getSettingValues(
                    $settingID
                );
                $setNo = false;
                foreach ($statesmeta as $k) {
                    if ($k->keytext == "Cron") {
                        $EcsShipmentSettings->updateSettingsValues(
                            $k->id,
                            $Cron
                        );
                    }
