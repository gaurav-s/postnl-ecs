    ...
    if (!empty($settingID)) {
        ...
        foreach ($statesmeta as $k) {
            if ($k->keytext == "giftcard_attribute") {
                ...
                $giftMessageAttribute = preg_replace(
                    "/\s+/",
                    "-",
        ...
        if (
            !isset($giftMessageAttribute) ||
            !$giftMessageAttribute ||
            empty($giftMessageAttribute)
        ) {
            return;
        }

        if (
            !isset($giftMessageAttributeValue) ||
            !$giftMessageAttributeValue ||
            empty($giftMessageAttributeValue)
        ) {
        ...
        echo '<table class="variations postnlecs-card" style="display:none;" cellspacing="0">
                    <tbody>
                        <tr>
                        <td class="label"><label for="postnl-fulfilment-card"> ' .
            __("Message for the card", "woocommercepostnlfulfillment") .
            '</label></td>
                        <td class="value">
                             <input id="postnl-fulfilment-card" type="text" name="card-text-message" style="width:100%;" value="' .
            $message .
            '" />
...
function woocommerce_postnl_fulfillment_card_message_validation()
{
    ...
    if (!empty($_REQUEST["card-text-message"])) {
        if (strlen($_REQUEST["card-text-message"]) > 255) {
            wc_add_notice(
                __(
                    "Card message should be less than 255 characters",
...
function woocommerce_postnl_fulfillment_save_card_message_field(
    $cart_item_data,
    $product_id
) {
    if (
        isset($_REQUEST["card-text-message"]) &&
        !empty(trim($_REQUEST["card-text-message"]))
    ) {
        $cart_item_data["card_message_text"] = sanitize_text_field(
            $_REQUEST["card-text-message"]
        );
...
function woocommerce_postnl_fulfillment_render_meta_on_cart_and_checkout(
    $cart_data,
    $cart_item = null
) {
    $custom_items = [];
    ...
    if (
        isset($cart_item["card_message_text"]) &&
        !empty(trim($cart_item["card_message_text"]))
    ) {
        $custom_items[] = [
            "name" => __(
            ...
            "value" => $cart_item["card_message_text"],
        ];
    }
    return $custom_items;
}
 ...
 */
function woocommerce_postnlfulfillment_card_message_order_meta_handler(
    $item,
    $cart_item_key,
    $values,
    $order
    ...
    if (isset($values["card_message_text"])) {
        $item->update_meta_data(
            "card_message_text",
            sanitize_text_field($values["card_message_text"])
        );
    }
}
