...
function getPostNLEcsShippingCode($shippingCountry, $order)
{
    ...
    $saoArray = ["Morning10", "Morning", "Morning12", "Evening", "Standard"];
    if ($shippingOptions) {
        $shipmentOptions = "";
        ...
        if (
            isset($shipmentOptions["only_recipient"]) &&
            $shipmentOptions["only_recipient"] != 0
        ) {
            $homeAddressOnly = "_SAO";
        }

        if (
            isset($shipmentOptions["signature"]) &&
            $shipmentOptions["signature"] != 0
        ) {
            $sinatureOption = "_SIG";
        }
        ...
        if (isset($shippingOptions["packageType"])) {
            ...
            if (
                $shippingOptions["packageType"] == "letter" ||
                $shippingOptions["packageType"] == "digital_stamp"
            ) {
                return "NA";
            }

            if ($shippingOptions["packageType"] == "package") {
                $postNlCode = getpostnlMappingCodes(
                    $shippingOptions,
                    $shippingCountry
                );
                if (in_array($postNlCode, $saoArray)) {
                    $postNlCode =
                        $postNlCode . $sinatureOption . $homeAddressOnly;
                }
                return $postNlCode;
            }
        }
...
function getpostnlMappingCodes($options, $countryCode)
{
    if (isset($options["deliveryType"])) {
        if (
            isset($options["isPickup"]) &&
            $options["deliveryType"] == "pickup" &&
            $options["isPickup"]
        ) {
...
function postnl_fulfilment_shipping_age_check($shippingCountry, $order)
{
    ...
    if (
        isset($shippingOptions["shipmentOptions"]) &&
        $shippingOptions["shipmentOptions"]
    ) {
        $shipmentOptions = $shippingOptions["shipmentOptions"];
        if (
            isset($shipmentOptions["age_check"]) &&
            $shipmentOptions["age_check"] != 0
        ) {
