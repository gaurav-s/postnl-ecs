    ...
    public function processOrders()
    {
        try {
            ...
            if ($settingID) {
                ...
                $this->log("Order Settings not found");
                return;
            }
            ...
            foreach ($Orderchunck as $order_split) {
                ...
                $message = $xml->createElementNS(
                    "http://www.toppak.nl/deliveryorder_new",
                    "message"
                );
                ...
                foreach ($order_split as $order) {
                    ...
                    if ($order_shipping_method_id == "l ") {
                        $order_shipping_method_id = "disabled";
                    }
                    ...
                    if (in_array($order_shipping_method_id, $shipements)) {
                        ...
                        if (strlen($order->get_id()) == 0) {
                            ...
                            if (strlen($order->get_id()) > 10) {
                                $failed->addError(
                                    " orderNo length is greater than 10 characters"
                                );
                                $isvalidate = false;
                            }
                        }
                        ...
                        $node->appendChild(
                            $xml->createElementNS(
                                "http://www.toppak.nl/deliveryorder_new",
                                "orderTime",
                                $order->get_date_created()->format("H:i:s")
                            )
                        );
                        $node->appendChild(
                            $xml->createElementNS(
                        ...
                        if (strlen($order->get_shipping_last_name()) == 0) {
                            ...
                            if (strlen($order->get_shipping_last_name()) > 35) {
                                $failed->addError(
                                    " shipping_last_name length is greater than 35 characters"
                                );
                        ...
                        if (strlen($order->get_shipping_first_name()) > 30) {
                            $failed->addError(
                                " shipToFirstName length is greater than 30"
                            );
                        ...
                        $orderShipPoststreet = trim(
                            $order->get_shipping_address_1()
                        ); //Set from WC
                        $orderShipPoststreetNum = trim(
                        ...
                        if (strlen($orderShipPostCode) == 0) {
                            $failed->addError(
                                " shipToPostalCode length is null"
                            );
                            $isvalidate = false;
                        } else {
                            if (strlen($orderShipPostCode) > 10) {
                                $failed->addError(
                                    " shipping_postcode length is greater than 10 characters"
                                );
                                $isvalidate = false;
                            }
                        }
                        ...
                        if (strlen($orderShipPostcity) == 0) {
                            ...
                            if (strlen($orderShipPostcity) > 30) {
                                $failed->addError(
                                    " shipping_city length is greater than 30 characters"
                                );
                                $isvalidate = false;
                            }
                        }
                        ...
                        if (strlen($orderShipPostcountry) == 0) {
                            $failed->addError(
                                " shipToCountryCode length is null"
                            );
                            $isvalidate = false;
                        } else {
                            if (strlen($orderShipPostcountry) > 2) {
                                $failed->addError(
                                    " shipToCountryCode length is greater than 2 characters"
                                );
                                $isvalidate = false;
                            }
                        }
                        ...
                        //SHIPPING PHONE
                        $billingPhoneOr = $order->get_billing_phone()
                            ? $order->get_billing_phone()
                            : "1";
                        $billingPhone = preg_replace(
                            "/\s+/",
                            "",
                            $billingPhoneOr
                        );
                        ...
                        if (strlen($billingPhone) > 15) {
                            $failed->addError(
                                " shipping_phone length is greater than 15 characters"
                            );
                            $isvalidate = false;
                        }
                        ...
                        if (strlen($orderShipPoststreet) == 0) {
                            $failed->addError(
                                " shipping_address_1 length is null"
                            );
                            $isvalidate = false;
                        } else {
                            if (
                                ...
                                $failed->addError(
                                    " shipToStreetHouseNrExt length is greater than 100 characters"
                                );
                                $isvalidate = false;
                            }
                        }
                        ...
                        $node->appendChild(
                            $xml->createElementNS(
                                ...
                                strlen($orderShipPoststreetNum) > 0
                                    ? $orderShipPoststreet .
                                        " " .
                                        $orderShipPoststreetNum
                                    : $orderShipPoststreet
                        ...
                        if (strlen($order->get_billing_email()) == 0) {
                            ...
                            if (strlen($order->get_billing_email()) > 50) {
                                $failed->addError(
                                    " billing_email length is greater than 50 characters"
                                );
                                $isvalidate = false;
                            }
                        }
                        ...
                        if (strlen($order->get_billing_last_name()) == 0) {
                            $failed->addError(
                                " invoiceToFirstName length is null"
                            );
                            $isvalidate = false;
                        } else {
                            if (strlen($order->get_billing_last_name()) > 35) {
                                $failed->addError(
                                    " billing_last_name length is greater than 35 characters"
                                );
                                $isvalidate = false;
                            }
                        }
                        ...
                        if (strlen($order->get_billing_first_name()) > 35) {
                            $failed->addError(
                                " billing_first_name length is greater than 35 characters"
                            );
                            $isvalidate = false;
                        }
                        ...
                        if (strlen($order->get_billing_company()) > 35) {
                            $failed->addError(
                                " billing_company length is greater than 35 characters"
                            );
                            $isvalidate = false;
                        }
                        ...
                        if (strlen($order->get_billing_postcode()) == 0) {
                            $failed->addError(
                                " invoiceToPostalCode length is null"
                            );
                            $isvalidate = false;
                        } else {
                            if (strlen($order->get_billing_postcode()) > 10) {
                                $failed->addError(
                                    " billing_postcode length is greater than 10 characters"
                                );
                                $isvalidate = false;
                            }
                        }
                        ...
                        if (strlen($order->get_billing_country()) == 0) {
                            $failed->addError(
                                " invoiceToCountryCode length is null"
                            );
                            $isvalidate = false;
                        } else {
                            if (strlen($order->get_billing_country()) > 2) {
                                $failed->addError(
                                    " billing_country Code length is greater than 2 characters"
                                );
                                $isvalidate = false;
                            }
                        }
                        ...
                        if (strlen($order->get_billing_address_1()) == 0) {
                            $failed->addError(
                                " billing_address_1 length is null"
                            );
                            $isvalidate = false;
                        } else {
                            if (
                                ...
                                $failed->addError(
                                    " BillingToStreetHouseNrExt length is greater than 100 characters"
                                );
                                $isvalidate = false;
                            }
                        }
                        ...
                        $node->appendChild(
                            $xml->createElementNS(
                                ...
                                strlen(trim($order->get_billing_address_2())) >
                                0
                                    ? trim($order->get_billing_address_1()) .
                                        " " .
                                        trim($order->get_billing_address_2())
                                    : trim($order->get_billing_address_1())
                            )
                        );
                        ...
                        if (strlen($order->get_billing_email()) > 50) {
                            $failed->addError(
                                " invoiceToEmail length is greater than 50"
                            );
                            $isvalidate = false;
                        }
                        ...
                        $node->appendChild(
                            $xml->createElementNS(
                                "http://www.toppak.nl/deliveryorder_new",
                                "language",
                                $newstring = substr(get_locale(), -2, 2)
                            )
                        );
                        $node->appendChild(
                            $xml->createElementNS(
                        ...
                        if (!$shippingCodePostNL) {
                            if (
                                ...
                                $order_shipping_method_id = get_outside_nl_shipping(
                                    $order->get_shipping_country()
                                );
                            }
                        } else {
                            //From PostNL-WooCommerce Plugin
                            $order_shipping_method_id = $shippingCodePostNL;
                        }

                        //Add Age Check option for NL
                        $ageCheckoption = postnl_fulfilment_shipping_age_check(
                            $order->get_shipping_country(),
                            $order
                        );

                        if (
                            !empty($ageCheckoption) &&
                            strtolower($order->get_shipping_country()) === "nl"
                        ) {
                        ...
                        foreach ($items as $item) {
                            ...
                            if (strlen($productSKU) == 0) {
                                $failed->addError(
                                    " No SKU Found in the ordered Item"
                                );
                                $isvalidate = false;
                            } elseif (strlen($productSKU) > 24) {
                                $failed->addError(
                                    " Item SKU length is greater than 24 characters"
                                );
                                $isvalidate = false;
                            }
                            ...
                            if (strlen($item["qty"]) == 0) {
                                ...
                                if (strlen($item["qty"]) > 5) {
                                    $failed->addError(
                                        " quantity length is greater than 5 characters"
                                    );
                                    $isvalidate = false;
                                }
                            }
                            ...
                            $orderItemNameClean = str_replace(
                                $this->_getBadCharacters(),
                                "",
                                $orderItemName
                            );

                            $line = $xml->createElementNS(
                                "http://www.toppak.nl/deliveryorder_new",
                                "deliveryOrderLine"
                            );
                            $line->appendChild(
                            ...
                            $line->appendChild(
                                $xml->createElementNS(
                                    "http://www.toppak.nl/deliveryorder_new",
                                    "singlePriceInclTax",
                                    $this->formatnumber($item["line_subtotal"])
                                )
                            );
                        ...
                        if ($isvalidate == true) {
                            ...
                            foreach ($FailedOrders as $fails) {
                                foreach ($fails->get_errors() as $fail) {
                                    $this->log('POSTNL ECS Failed Order ID:  '.$fails->get_orderID().' '.$fail);
                                }
                            }
                ...
                if ($isEmpty > 0) {
                    ...
                    if (!$is_valid_xml) {
                        ...
                        $failed->set_orderID("");
                        $failed->addError(
                            " Order XML is invalid: " . $validationError
                        );
                        array_push($FailedOrders, $failed);
                    } else {
                        if (function_exists("libxml_use_internal_errors")) {
