...
class PostNLShipment extends PostNLProcess
{
    public function processShipment()
    {
        try {
            ...
            if ($settingID) {
                $statesmeta = $EcsShipmentSettings->loadShipmentSettings(
                    $settingID
                );
            } else {
            ...
            foreach ($sftp->nlist() as $filename) {
                ...
                if (count($codesNames) > 0) {
                    ...
                    if ($shipmentFileData) {
                        ...
                        if (
                            strcmp(trim($xmlRetailname), trim($nameRetailer)) ==
                            0
                        ) {
                            $validate = true;
                        } else {
                            ...
                            array_push(
                                $inventory_errors,
                                "The retailer name from the shipment message and the system configuration do not match"
                            );
                        }
                        ...
                        foreach ($xml->orderStatus as $stock) {
                            ...
                            if (
                                false === get_post_status((int) $stock->orderNo)
                            ) {
                                $validate = false;
                                array_push(
                                    $inventory_errors,
                                    "Order  ID :" .
                                        $stock->orderNo .
                                        "  is not found for the shipment"
                                );
                                continue; // Skip further check
                            }

                            $processedFiles = get_post_meta(
                                $intOrder,
                                "shipmentFiles",
                                true
                            );
                            if (!empty($processedFiles)) {
                                $processedFilesArray = json_decode(
                                    $processedFiles
                                );
                        ...
                        if (
                            ...
                            foreach ($xml->orderStatus as $stock) {
                                ...
                                foreach ($items as $orderlineItem) {
                                    ...
                                    if (
                                        $lineItemProduct->is_downloadable("yes")
                                    ) {
                                        continue;
                                    }
                                ...
                                if (
                                    ...
                                    $existingtrackCode = get_post_meta(
                                        $intOrder,
                                        "trackAndTraceCode",
                                        true
                                    );
                                ...
                                if ($countElement == $ordExportedItems) {
                                    ...
                                    $exportedItems = get_post_meta(
                                        $intOrder,
                                        "exportedItems",
                                        true
                                    );
                                    if (strlen($exportedItems) !== 0) {
                                        $itemsExported = explode(
                                            ":",
                                            $exportedItems
                                        );
                                        $itemsExportedNewly = explode(
                                            ":",
                                            $shippedOrders_ids
                                        );
                                        $totalItems =
                                            count($itemsExported) +
                                            count($itemsExportedNewly) -
                                            2;
                                ...
                                $processedFilesJson = json_encode(
                                    $processedFilesArray
                                );
